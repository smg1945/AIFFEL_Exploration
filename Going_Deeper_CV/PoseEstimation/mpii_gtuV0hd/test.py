...
import cv2
import numpy as np
from mpii_gtuV0hd.hourglass104 import StackedHourglassNetwork
from mpii_gtuV0hd.simplebaseline import Simplebaseline
...
model_hour = StackedHourglassNetwork(
        input_shape=(256, 256, 3), num_stack=4, num_residual=1,
        num_heatmap=16)
model_simple = Simplebaseline((256, 256, 3))

model_hour.load_weights('./models/model-v0.0.1-epoch-3-loss-9.7749.h5')
model_simple.load_weights('./models/model-v0.0.1-epoch-3-loss-6.0714.h5')
...
def predict_hour(image_path):
    encoded = tf.io.read_file(image_path)
    image = tf.io.decode_jpeg(encoded)
    inputs = tf.image.resize(image, (256, 256))
    inputs = tf.cast(inputs, tf.float32) / 127.5 - 1
    inputs = tf.expand_dims(inputs, 0)
    outputs = model_hour(inputs, training=False)
...
def predict_simple(image_path):
    ...
    inputs = tf.cast(inputs, tf.float32) / 127.5 - 1
    inputs = tf.expand_dims(inputs, 0)
    outputs = model_simple(inputs, training=False)
    heatmap = outputs[0].numpy()
    kp = extract_keypoints_from_heatmap(heatmap)
    return image, kp
