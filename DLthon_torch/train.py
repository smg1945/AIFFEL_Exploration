...
def load_names(path, names_train=True):
    if names_train:
        images = glob(os.path.join(path + '/data' + '/train' + "/PNG" + '/Originals' + "/*.png"))
        masks = glob(os.path.join(path + '/data' + '/train' + "/PNG" + '/GroundTruth' + "/*.png"))
    else:
        images = glob(os.path.join(path + '/data' + '/test' + '/images' + "/9.bmp"))
        masks = glob(os.path.join(path + '/data' + '/test' + '/labels' + "/9.bmp"))
    return images, masks

def load_data(path, is_train=True, names_train=True):
    if is_train:
        train_x, train_y = load_names(path, names_train)
        valid_x, valid_y = load_names(path, names_train)
...
def train(model, loader, optimizer, loss_fn, device):
    ...
    epoch_iou = epoch_iou/len(loader)

    return epoch_loss, [epoch_jac, epoch_f1, epoch_recall, epoch_precision, epoch_iou]
...
if __name__ == "__main__":
    ...
    """ Dataset """
    (train_x, train_y, train_label), (valid_x, valid_y, valid_label) = load_data(path, is_train=True)
